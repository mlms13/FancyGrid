...
var StringVsArrayCache = function() { };
StringVsArrayCache.__name__ = ["StringVsArrayCache"];
StringVsArrayCache.main = function() {
	var _g = [];
	var _g2 = 0;
	while(_g2 < 500) {
		var x = _g2++;
		...
		while(_g4 < 500) {
			var y = _g4++;
			_g.push({ x : x, y : y, value : "" + x + "," + y});
		}
	}
	StringVsArrayCache.testSetCache(_g);
	StringVsArrayCache.testGetCache(_g);
};
StringVsArrayCache.testSetCache = function(tests) {
	var suite = new thx_benchmark_speed_Suite(10,2500);
	suite.addCase("set array cache",function(__counter__) {
		var __timer__ = new thx_benchmark_measure_Stopwatch();
		var cache = new ArrayCache();
		__timer__.start();
		while(--__counter__ >= 0.0) {
			var _g = 0;
	...
	suite.addCase("set map cache",function(__counter__1) {
		var __timer__1 = new thx_benchmark_measure_Stopwatch();
		var cache1 = new MapCache();
		__timer__1.start();
		while(--__counter__1 >= 0.0) {
			var _g1 = 0;
			while(_g1 < tests.length) {
				var t1 = tests[_g1];
				++_g1;
				var _this = cache1.map;
				var key = "" + t1.x + "-" + t1.y;
				var value = t1.value;
	...
	suite.addCase("set double map cache",function(__counter__2) {
		var __timer__2 = new thx_benchmark_measure_Stopwatch();
		var cache2 = new DoubleMapCache();
		__timer__2.start();
		while(--__counter__2 >= 0.0) {
			var _g2 = 0;
			while(_g2 < tests.length) {
				var t2 = tests[_g2];
				++_g2;
				cache2.set(t2.x,t2.y,t2.value);
			}
		}
		__timer__2.stop();
...
StringVsArrayCache.testGetCache = function(tests) {
	var suite = new thx_benchmark_speed_Suite(10,2500);
	...
	while(_g2 < tests.length) {
		var t2 = tests[_g2];
		++_g2;
		cache3.set(t2.x,t2.y,t2.value);
	}
	suite.addCase("get array cache",function(__counter__) {
		var __timer__ = new thx_benchmark_measure_Stopwatch();
		var buf = [];
		__timer__.start();
		while(--__counter__ >= 0.0) {
	...
	suite.addCase("get map cache",function(__counter__1) {
		var __timer__1 = new thx_benchmark_measure_Stopwatch();
		var buf1 = [];
		__timer__1.start();
		while(--__counter__1 >= 0.0) {
			...
			while(_g12 < _g4) {
				...
				while(_g32 < _g22) {
					var y1 = _g32++;
					var _this1 = cache2.map;
					var key1 = "" + x1 * y1 + "-" + y1 % x1;
					buf1.push(__map_reserved[key1] != null?_this1.getReserved(key1):_this1.h[key1]);
				}
			}
		}
		__timer__1.stop();
	...
	suite.addCase("get double map cache",function(__counter__2) {
		var __timer__2 = new thx_benchmark_measure_Stopwatch();
		var buf2 = [];
		__timer__2.start();
		while(--__counter__2 >= 0.0) {
			...
			while(_g13 < _g5) {
				...
				while(_g33 < _g23) {
					var y2 = _g33++;
					buf2.push(cache3.get(x2 * y2,y2 % x2));
				}
			}
		}
		__timer__2.stop();
...
ArrayCache.__name__ = ["ArrayCache"];
ArrayCache.prototype = {
	...
	,__class__: ArrayCache
};
var MapCache = function() {
	this.map = new haxe_ds_StringMap();
};
MapCache.__name__ = ["MapCache"];
MapCache.prototype = {
	map: null
	,k: function(x,y) {
		return "" + x + "-" + y;
...
var DoubleMapCache = function() {
	this.map = new haxe_ds_IntMap();
};
DoubleMapCache.__name__ = ["DoubleMapCache"];
DoubleMapCache.prototype = {
	map: null
	,get: function(x,y) {
		var m = this.map.h[y];
		if(null == m) {
	...
	,set: function(x,y,v) {
		var m = this.map.h[y];
		if(null == m) {
			m = new haxe_ds_IntMap();
			this.map.h[y] = m;
			return;
		}
		m.h[x] = v;
	}
	,__class__: DoubleMapCache
};
	...
	,toString: function() {
		...
		return "\n" + values.map(function(row2) {
			...
			while(_g11 < _g2) {
				var i1 = _g11++;
				var cell = row2[i1];
				r.push(cell == ""?thx_Strings.rpad("",hb,cols[i1] + 2):align[i1]?" " + thx_Strings.rpad(cell," ",cols[i1]) + " ":" " + thx_Strings.lpad(cell," ",cols[i1]) + " ");
			}
			return vb + r.join(vb) + vb;
		}).join("\n");
	}
